...
export class fixTables1678265843231 implements MigrationInterface {
    name = 'fixTables1678265843231'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`group_cards\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`title\` varchar(100) NOT NULL, \`content\` varchar(255) NOT NULL, \`tag\` varchar(100) NOT NULL, \`thumbnail\` varchar(20) NOT NULL, \`order\` smallint NOT NULL, \`is_favorite\` tinyint NOT NULL COMMENT '0: NONE | 1: FAVORITED' DEFAULT '0', \`column_id\` varchar(36) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`group_cards\` ADD CONSTRAINT \`FK_06b3b255e58b0c5b7bb32e9c60b\` FOREIGN KEY (\`column_id\`) REFERENCES \`group_columns\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`answers\` DROP FOREIGN KEY \`FK_cb080abe9c2f19dc80f9563bf50\``);
        await queryRunner.query(`ALTER TABLE \`group_cards\` DROP FOREIGN KEY \`FK_06b3b255e58b0c5b7bb32e9c60b\``);
        ...
        await queryRunner.query(`ALTER TABLE \`user_tokens\` CHANGE \`refresh_expire_at\` \`refresh_expire_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP()`);
        await queryRunner.query(`ALTER TABLE \`user_tokens\` CHANGE \`expire_at\` \`expire_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP()`);
        await queryRunner.query(`DROP TABLE \`group_cards\``);
    }
